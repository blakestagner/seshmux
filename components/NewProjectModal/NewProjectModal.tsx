...
//
// The Location field (FolderField) opens the real OS folder chooser — the server
// runs on this machine, so it can (see server/lib/folder-picker.ts); the
// browser's own picker is sandboxed and returns a handle, never a path. It
// selects the PROJECT FOLDER, so the dialog's own "New Folder" button creates
// one; the pick is split back into Location + Folder name, both still editable.
// Where no dialog can be shown (headless host, no zenity/kdialog) the typed path
// is the whole story: seeded with the home dir, with the parents of existing
// projects offered as a datalist so the common case stays one click.

import { useEffect, useState } from 'react';
import { createProjectFolder, getHomeDir } from '../../lib/client/api';
...
export default function NewProjectModal({ providers, suggestions, onCreate, onClose }: NewProjectModalProps) {
  ...
  // The chooser picks the PROJECT FOLDER itself, not its parent — the OS dialog's
  // own "New Folder" button is then the whole create flow, which is the point of
  // having a native picker at all. The result is split back into the two fields
  // so it stays visible and editable before Create.
