...
- **Embedded browser / dev-server preview** → `server/lib/preview.ts` (port discovery), `server/lib/dev-script.ts` (package.json → run command), `server/routes/preview.ts`, `components/BrowserPanel/`. Port discovery here is deliberately NOT `lib/ports.ts`: that one resolves a port to a process via lsof and returns `[]` on win32 forever, so the browser panel instead scrapes localhost URLs out of the session's own PTY scrollback and TCP-probes them. Anything that must work on Windows should ask that question, not the lsof one.
