...
export interface MemoryDeps {
  ...
  /**
   * Called when memory is first READ, to arm the store watcher.
   *
   * Arming only on mutation was useless for the case the watcher exists for: an agent's
   * `remember` arrives from the mcp-bridge PROCESS, which never touches these routes, so
   * nothing would ever have armed it before that write. Same shape as the scratchpad
   * route's onOpen, which arms from its GET for the same reason.
   */
  onOpen?: () => void;
  /**
   * Runs a store-rewriting operation exclusively against the harvester.
   *
   * compact() rewrites shards from a snapshot, so an append landing between its read and
   * its rename is lost. The harvester owns the only serial queue in the process; without
   * borrowing it, store.ts's documented single-writer invariant is not actually enforced.
   */
...
export default async function memoryRoutes(
  ...
  f.get("/api/memory", async (req, reply) => {
    deps.onOpen?.();
    const q = req.query as Record<string, unknown>;
    const scope = (q.scope === "all" ? "all" : "project") as MemoryScopeMode;
    const project = typeof q.project === "string" ? q.project : undefined;
    // Refuse rather than quietly search everything: the caller asked for one repo.
    if (scope === "project" && !project) {
      return reply
        .code(400)
        .send({ error: "project is required unless scope=all" });
    }
    const result = await recall(
      {
        ...
        provider:
          typeof q.provider === "string" ? (q.provider as never) : undefined,
    ...
    return {
      rows: result.ranked.map((r) => toRow(r.record, r.score)),
  ...
  f.post("/api/memory/pack", async (req, reply) => {
    ...
    if (chosen.length === 0)
      return reply.code(404).send({ error: "no such records" });
    ...
    const packed = pack(
      ...
      {
        budgetTokens:
          num(body.budgetTokens) ?? DEFAULT_MEMORY_SETTINGS.recallBudgetTokens,
    ...
    await appendOverlay(
      packed.records.map((r) => ({ op: "hit" as const, id: r.id, at: now })),
    );
    changed();
    return {
      text: packed.text,
  ...
  f.post("/api/memory", async (req, reply) => {
    ...
    if (!projectId || !text.trim())
      return reply.code(400).send({ error: "projectId and text required" });
    ...
    changed(projectId);
    if (!res.record)
      return reply.code(200).send({ written: false, note: res.note });
  ...
  f.patch("/api/memory/:id", async (req, reply) => {
    ...
    if (!all.some((r) => r.id === id))
      return reply.code(404).send({ error: "no such record" });

    const ops = [];
    if (typeof body.pinned === "boolean")
      ops.push({
        op: body.pinned ? ("pin" as const) : ("unpin" as const),
        id,
        at: now,
      });
    if (typeof body.text === "string" && body.text.trim())
      ops.push({ op: "edit" as const, id, at: now, text: body.text });
    if (ops.length === 0)
  ...
  f.post("/api/memory/distill", async (req, reply) => {
    ...
    if (!projectId || !sessionId)
      return reply
        .code(400)
        .send({ error: "projectId and sessionId required" });

    const project = await resolveProject(projectId);
    if (!project) return reply.code(404).send({ error: "unknown project" });

    // Validate against the registry rather than coercing anything non-'codex' to 'claude':
    // that turned a typo into a silent agent spend against the WRONG provider.
    const providers = await getProviders();
    const requested =
      typeof body.provider === "string" ? body.provider : providers[0]?.id;
    const provider = providers.find((p) => p.id === requested);
    if (!provider)
      return reply
        .code(400)
        .send({ error: `provider ${requested} not available` });
  ...
  f.post("/api/memory/compact", async (req) => {
    const body = (req.body ?? {}) as Record<string, unknown>;
    const run = deps.serialize ?? (<T>(fn: () => Promise<T>) => fn());
    const res = await run(() =>
      compact(
        {
          ...DEFAULT_MEMORY_SETTINGS,
          retentionDays:
            num(body.retentionDays) ?? DEFAULT_MEMORY_SETTINGS.retentionDays,
          maxRecords:
            num(body.maxRecords) ?? DEFAULT_MEMORY_SETTINGS.maxRecords,
        },
        deps.now?.(),
      ),
    );
