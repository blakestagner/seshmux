...
// can't launch a batch script, so a .cmd/.bat target must run under cmd.exe. This
// returns [comspec, ['/d','/c', <command line>], opts] — `/d` disables any HKCU
// AutoRun. Identity ([file, args, {}]) on posix or for a non-.cmd/.bat file.
//
...
// child_process honors opts.windowsVerbatimArguments; node-pty has no such option but
// takes a pre-escaped command line when `args` is a STRING (windowsPtyAgent.js:
...
//    & | < > ( ) inside double quotes, so the quotes ARE the neutralization — carets
//    would NOT work there (inside quotes a ^ is literal and would be delivered to the
//    program verbatim, corrupting the argument).
...
/** Spawn options the caller MUST apply for the returned command line to survive. */
export type CmdSpawnOpts = { windowsVerbatimArguments?: true };
...
// Prefix cmd.exe metacharacters with ^ so cmd passes them to the program instead of
// acting on them. cmd consumes the ^ before the program sees the line. Only valid for
// an UNQUOTED token — see cmdToken.
export function cmdEscape(line: string): string {
  return line.replace(/[()%!^"<>&|]/g, '^$&');
}

// One token of the cmd command line: quoted OR caret-escaped, never both.
export function cmdToken(arg: string): string {
  const quoted = quoteArgv(arg);
  return quoted === arg ? cmdEscape(arg) : quoted;
}

export function cmdInvocation(file: string, args: string[]): [string, string[], CmdSpawnOpts] {
  if (process.platform !== 'win32' || !/\.(cmd|bat)$/i.test(file)) return [file, args, {}];
  const inner = [file, ...args].map(cmdToken).join(' ');
  const comspec = process.env.ComSpec || 'cmd.exe';
  // The outer quotes are load-bearing: `cmd /c` strips the FIRST and LAST quote of its
  // remainder, so wrapping is what lets an inner "quoted path with spaces" survive that
  // strip intact. With no args and an unspaced file the wrap is inert (cmd strips it
  // straight back off), so the common case is byte-identical to the line node used to
  // build.
